...
export async function callDifyChatStream(params: {
  ...
  const { query, user, conversationId, inputs, agentId, signal, files, sessionId } = params
  ...
  const API_BASE_URL = '/api'//process.env.NODE_ENV === 'development' ? 'http://192.168.11.95:6039/' : '/api'//? 'http://192.168.10.66:6039' : '/api'//'http://192.168.11.126:26039' : '/api'//'http://192.168.10.66:6039' : '/api'
...
export async function stopDifyTask(params: {
  ...
  try {
    ...
    const API_BASE_URL = '/api'//process.env.NODE_ENV === 'development' ? 'http://192.168.10.66:6039/api' : '/api'
